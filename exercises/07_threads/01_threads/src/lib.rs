...
pub fn sum(v: Vec<i32>) -> i32 {
    let half_position = v.len() / 2;
    let left = &v[..half_position];
    let right = &v[half_position..];
    let left_clone: Vec<i32> = left.iter().cloned().collect();
    let handle = thread::spawn(move || left_clone.iter().sum::<i32>());
