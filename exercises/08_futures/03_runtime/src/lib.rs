    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let first_handle = tokio::spawn(reply_string(first, reply_arc.clone()));
    let second_handle = tokio::spawn(reply_string(second, reply_arc));
    tokio::join!(first_handle, second_handle);
}

async fn reply_string<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
{
    loop {
